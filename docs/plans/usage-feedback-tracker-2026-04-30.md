...
AFS should feel like a workspace-first remote filesystem that can be mounted
to, unmounted from, inspected, and checkpointed. The current surface still leaks
...
2. Local data-loss fear from `afs ws unmount` deleting a directory by default.
...
- Prefer `mount` and `unmount` for local lifecycle.
...
- [CLI Mount/Unmount Command Surface Plan](cli-mount-unmount-command-surface.md)
...
2. Make `unmount` semantics safe by default.
...
- Verify `afs ws unmount`/session cleanup cannot resolve across unauthorized
  workspace routes.
- Verify local directories survive normal unmount/down.

### Wave 1: First Real Local Workflow

Goal: make the desired workflow obvious and working.

1. Add `afs ws mount <workspace> <directory>`.
2. Add `afs ws unmount <directory> [--delete]`.
...
6. Rework docs/help/install next steps around mount/unmount.

Validation:

- Clean HOME CLI tests for mount/unmount parsing and behavior.
- End-to-end import existing directory, mount, edit file, unmount, remount.
- Non-TTY output snapshots for scriptability.

### Wave 2: Inspect Before Mount

Goal: let users inspect a remote workspace without a local mount.
...
4. Keep this distinct from local mount/unmount lifecycle.
...
4. Support multiple mounted workspaces per daemon or per machine.
...
| F010 | P1 | Mental model | Workspace/database/directory/agent model is not immediately obvious. | Product copy leak. | Define: workspace = durable file tree, database = storage backing, mounted directory = local working surface, agent = connected client/daemon. Hide databases by default. | Planned |
...
| F012 | P1 | Local workflow | User wanted to create a workspace from an existing directory and mount that same directory. Could not find clear path. | Core first-real-use gap. | Make `afs ws mount <workspace> <dir>` canonical. Rework import/mount flow so existing local directory can become the mounted working directory safely. | Planned |
| F013 | P0 | Local data loss | `afs ws unmount` deletes a directory on unmount. Scary. Should require `--delete`; default should unmount. | Accepted as trust/data-loss issue. | Add `afs ws unmount`. Make default preserve local files. Make `--delete` explicit and path-guarded. Transition `down` to unmount behavior. | Planned |
| F014 | P1 | Vocabulary | `mount`/`unmount` feels like a better metaphor than lifecycle start/stop language. | Accepted. | Make `afs ws mount` / `afs ws unmount` the documented surface and update help/docs/install copy. | Planned |
| F015 | P1 | Large files | Large files appear ignored, probably safety/testing, with no logs. User thought it was broken. | Accepted. Silent skip is a bug. | Emit skipped-file rows with path, size, cap, and config key. Include skip count in mount/import/sync summaries and `afs log`. | Planned |
| F016 | P2 | Multi-mount | Allow daemon to support many mounted workspaces. Current model appears to support one. | Useful but bigger state model change. | Defer until single mount/unmount semantics are correct. Then replace single state file with mount records keyed by local path and workspace ID. | Deferred |
| F017 | P1 | Remote inspection | Allow remote inspection of an unmounted workspace before mounting. | Accepted. | Add or consolidate `afs fs -w <workspace> ...` style commands for ls/cat/find/grep. Reuse existing workspace tree/content/search APIs. | Planned |
...
| F019 | P2 | Remote fs commands | Proposed `afs fs -w agent1 ls/find/grep/cat`. | Accepted direction, exact command name open. | Design separate remote-inspection surface. Avoid conflating with mount/unmount. | Planned |
| F020 | P1 | Mount output | Proposed verbose mount operation table with import/download/upload/conflict/skipped codes. | Accepted. | Implement mount reconciliation summary and optional verbose per-path operation output. | Planned |
...
| F022 | P2 | Tags | Proposed `afs tag create agent1@<ts> pre-mount-v1`. | Useful, but should not become Git vocabulary. | Treat tags as checkpoint labels or aliases. Require checkpoint/timestamp resolution. Keep out of first mount/unmount slice. | Deferred |
| F023 | P0 | Tenant isolation | `afs ws unmount` error showed same workspace name across multiple databases with IDs. Looks like cross-tenant information leak. | Production blocker. | Enforce principal/org scoped workspace/session cleanup. Resolve by opaque workspace ID where possible. Scrub unauthorized IDs/names from errors. Add duplicate starter workspace tests. | Planned |
...
- Session cleanup called from `afs ws unmount`.
...
- `afs ws mount`
- `afs ws unmount`
- `afs ws unmount --delete`
- Help/docs/install/onboarding copy.

Deliverables:

- Safe default unmount behavior.
- Backward-compatible `up`/`down` routing during transition.
- No local deletion unless runtime state proves the path is mounted and
  `--delete` is present.
...
- Workspace filesystem commands without mount.
...
- Do not implement multi-mount before single mount/unmount is safe.
...
1. Should `afs ws unmount` accept `--delete`, or should destructive behavior exist
   only as `afs ws unmount <dir> --delete`?
2. Should remote inspection be named `afs fs`, or
   `afs ws files`?
5. Should `unmount --delete` require an interactive confirmation in TTY mode?
...
- Decide command names for `mount`, `unmount`, and optional `ws`.
- Decide whether the first mount implementation targets sync only.
