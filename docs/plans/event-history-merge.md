...
4. UI: collapse workspace `History` + `Changelog` into one `History` tab with a filter pill set (lifecycle on, file ops off by default). CLI `afs log` keeps its current filter (file ops only).
...
- `afs log` → unchanged behavior, now backed by `ListEvents(kind=file)`.
