...
```bash
afs versioning get [workspace]
afs versioning set [workspace] --mode off|all|paths \
  [--include 'src/**']... \
  [--exclude '**/*.log']... \
  [--max-versions-per-file 100] \
  [--max-age-days 30]

afs file history [workspace] <path>
afs file show [workspace] <path> --version <version-id>
afs file diff [workspace] <path> --from <version-id|head> --to <version-id|head>
afs file restore [workspace] <path> --version <version-id>
afs file undelete [workspace] <path> --version <version-id>
```
...
- `show` should print file content or a binary notice.
...
- `afs file restore` or `file_restore_version` writes the selected historical
...
`afs file history` should resolve the current or historical lineage for a path,
...
`afs file show` and `file_read_version` should fetch the exact stored version
...
`afs file undelete` should list the latest tombstoned lineage at that path by
...
- CLI: `versioning get/set`, `file history`, `file show`

### Phase 2

- CLI: `file diff`, `file restore`, `file undelete`
- HTTP routes
