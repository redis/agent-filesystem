...
func TestStoreRecordFileVersionMutationSeedsBaselineForExistingTrackedFile(t *testing.T) {
	...
	baselineContent, err := service.GetFileVersionContent(context.Background(), "repo", history.Lineages[0].Versions[0].VersionID)
	if err != nil {
		t.Fatalf("GetFileVersionContent(baseline) returned error: %v", err)
	}
	if baselineContent.Content != "before\n" {
		t.Fatalf("baseline content = %q, want %q", baselineContent.Content, "before\n")
	}
}
