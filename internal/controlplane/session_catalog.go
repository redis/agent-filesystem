...
func (c *workspaceCatalog) UpsertSession(ctx context.Context, item sessionCatalogRecord) error {
	...
	_, err := c.execContext(
		...
		strings.TrimSpace(item.AgentName),
		strings.TrimSpace(item.SessionName),
...
func (c *workspaceCatalog) ListAllSessions(ctx context.Context) ([]sessionCatalogRecord, error) {
	if c == nil || c.db == nil {
		return nil, nil
	}
