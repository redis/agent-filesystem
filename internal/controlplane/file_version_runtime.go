...
func (s *Store) RecordFileVersionMutation(ctx context.Context, workspace string, before, after VersionedFileSnapshot, metadata FileVersionMutationMetadata) (*FileVersion, error) {
	...
	if seedBaseline {
		if err := s.saveVersionedSnapshotBlob(ctx, workspace, before); err != nil {
			if createdLineage {
	...
	if err := s.saveVersionedSnapshotBlob(ctx, workspace, after); err != nil {
		return nil, err
	}
...
func (s *Store) saveVersionedSnapshotBlob(ctx context.Context, workspace string, snapshot VersionedFileSnapshot) error {
	if !snapshot.Exists || snapshot.Kind != "file" || snapshot.BlobID == "" || snapshot.Content == nil {
		return nil
	}
	return s.SaveBlobs(ctx, workspace, map[string][]byte{snapshot.BlobID: snapshot.Content})
}
