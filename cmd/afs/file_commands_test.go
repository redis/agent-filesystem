...
func TestFSFileCommandArgsKeepsWorkspaceSeparateFromPath(t *testing.T) {
	parsed, err := parseFileHistoryArgs(fsFileCommandArgs("history", "repo", []string{"/notes/app.txt"})[2:])
	if err != nil {
		t.Fatalf("parseFileHistoryArgs() returned error: %v", err)
	}
	if parsed.workspace != "repo" || parsed.path != "/notes/app.txt" {
		t.Fatalf("parsed = %#v, want workspace repo and path /notes/app.txt", parsed)
	}
}
...
func TestCmdFSHistoryReportsExistingFileWithoutRecordedHistory(t *testing.T) {
	t.Helper()
	...
	rootKey, _, _, err := controlplane.EnsureWorkspaceRoot(context.Background(), store.cp, "repo")
	if err != nil {
		t.Fatalf("EnsureWorkspaceRoot() returned error: %v", err)
	}
	if err := mountclient.New(store.rdb, rootKey).EchoCreate(context.Background(), "/helloworld.txt", []byte("hello from live file\n"), 0o644); err != nil {
	...
	for _, want := range []string{"workspace", "repo", "/helloworld.txt", "no history recorded yet"} {
		if !strings.Contains(output, want) {
			t.Fatalf("cmdFS(history) output = %q, want substring %q", output, want)
		}
	}
}
